...
class Block extends Model {
    ...
    async $beforeInsert() {
        ...
        if (!this.current_hash) {
            this.current_hash = crypto.createHash('sha256').update(`${this.index}${this.previous_hash}${this.timestamp.valueOf()}${this.data}`).digest("base64");
        }
    }
...
async function retrievePreviousBlock() {
    let previous = await Block.query().orderBy('timestamp', 'desc').first();
    ...
    if (!previous) {
        return {
            ...
            timestamp: 0, 
            data: '',
            current_hash: crypto.createHash('sha256').update("000").digest("base64"),
        }
    }
